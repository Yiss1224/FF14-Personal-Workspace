...
const DUNGEON_TC_CACHE_KEY='dungeonTcNamesV1';
const DUNGEON_TC_CACHE_MS=30*24*60*60*1000;
const DUNGEON_EN_CSV='https://raw.githubusercontent.com/xivapi/ffxiv-datamining/master/csv/en/ContentFinderCondition.csv';
const DUNGEON_TC_CSV='https://raw.githubusercontent.com/thewakingsands/ffxiv-datamining-tc/main/ContentFinderCondition.csv';
...
function dungeonCsvLine(line){const out=[];let cur='',quoted=false;for(let i=0;i<line.length;i++){const c=line[i];if(c==='"'){if(quoted&&line[i+1]==='"'){cur+='"';i++}else quoted=!quoted}else if(c===','&&!quoted){out.push(cur);cur=''}else cur+=c}out.push(cur);return out}
function dungeonNamesById(text){const lines=String(text||'').replace(/^\uFEFF/,'').split(/\r?\n/),headers=dungeonCsvLine(lines[1]||''),nameIndex=headers.indexOf('Name'),out={};if(nameIndex<0)return out;for(let i=3;i<lines.length;i++){if(!lines[i])continue;const row=dungeonCsvLine(lines[i]),id=Number(row[0]),name=String(row[nameIndex]||'').trim();if(Number.isFinite(id)&&name)out[id]=name}return out}
function dungeonNameKey(v){return String(v||'').toLowerCase().replace(/[’‘]/g,"'").replace(/\s*\([^)]*\)\s*$/,'').replace(/[^a-z0-9]+/g,' ').trim()}
function dungeonDisplayName(name){return dungeonTcNames[dungeonNameKey(name)]||name}
async function loadDungeonTcNames(force=false){
  try{
    const cached=JSON.parse(localStorage.getItem(DUNGEON_TC_CACHE_KEY)||'null');
    if(!force&&cached?.ts&&Date.now()-cached.ts<DUNGEON_TC_CACHE_MS&&cached.names&&typeof cached.names==='object'){
      dungeonTcNames=cached.names;renderDungeonPreview();return dungeonTcNames;
    }
  }catch{}
  try{
    const [enRes,tcRes]=await Promise.all([fetch(DUNGEON_EN_CSV,{cache:'no-store'}),fetch(DUNGEON_TC_CSV,{cache:'no-store'})]);
    if(!enRes.ok||!tcRes.ok)throw new Error(`HTTP ${enRes.status}/${tcRes.status}`);
    const [enText,tcText]=await Promise.all([enRes.text(),tcRes.text()]),en=dungeonNamesById(enText),tc=dungeonNamesById(tcText),map={};
...
// Leveling route: use the highest-level non-cap dungeon the job can currently enter.
// Do not depend on the Wiki roulette flag; that field is incomplete for some duties.
function bestDungeonForLevel(level){const eligible=dungeonExpCatalog.filter(d=>d.level<=level&&d.level<100&&d.baseExp>0&&d.level%10!==0);return eligible.slice().sort((a,b)=>b.level-a.level||b.baseExp-a.baseExp)[0]||null}
function dungeonGainForJob(job,d){return d?Math.round(d.baseExp*armouryMultiplier(job)):0}
function renderDungeonPreview(){const box=document.getElementById('dungeon-exp-preview');if(!box)return;if(!dungeonExpCatalog.length){box.innerHTML='<span class="muted">尚無副本 EXP 資料。</span>';return}const levels=[51,61,67,69,71,81,91,99],rows=levels.map(l=>{const d=bestDungeonForLevel(l);return d?`<tr><td>Lv${l}</td><td>${lvlEsc(dungeonDisplayName(d.name))}</td><td>Lv${d.level}</td><td>${Math.round(d.baseExp).toLocaleString('zh-TW')}</td></tr>`:''}).join('');box.innerHTML=`<table><tr><th>角色等級</th><th>自動推薦</th><th>副本等級</th><th>Base EXP</th></tr>${rows}</table><div class="muted">自動推薦會選目前可進的最高等級非滿等副本；同級再以 Base EXP 較高者優先。名稱使用台服官方繁中資料。</div>`}
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  Promise.resolve(loadDungeonTcNames(false)).catch(e=>console.error('initial dungeon TC name load failed',e));
});
