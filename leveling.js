...
function getLevelJobs(){
  const saved=store.get('levelJobs',null);
  let rows=Array.isArray(saved)?saved.filter(x=>x&&typeof x==='object').map(normalizeLevelJob):[];
  if(!rows.length)rows=LEVEL_JOB_DEFAULTS.map(x=>normalizeLevelJob({...x,id:lvlUid()}));
  return rows;
}
...
async function loadDungeonExpCatalog(force=false){
  ...
  try{
    ...
    const json=await r.json(),src=json?.query?.results||{},rows=[];
    for(const[key,val]of Object.entries(src)){const p=val?.printouts||{},exp=firstNumber(p['Has duty experience']),level=firstNumber(p['Has duty level requirement']);if(!exp||!level)continue;rows.push({name:val?.fulltext||key,level,baseExp:exp,roulette:textList(p['Is available for duty roulette']).join(', ')})}
    rows.sort((a,b)=>a.level-b.level||a.name.localeCompare(b.name));if(rows.length<10)throw new Error(`回傳資料筆數異常（${rows.length}）`);
  ...
  }catch(e){dungeonExpCatalog=Array.isArray(cached?.rows)?cached.rows:[];if(status)status.textContent=`副本 EXP 讀取失敗：${e.message}${dungeonExpCatalog.length?'；改用舊快取':''}`;console.error('dungeon EXP load failed',e);renderDungeonPreview();return dungeonExpCatalog}
}
...
function bestDungeonForLevel(level){const eligible=dungeonExpCatalog.filter(d=>d.level<=level&&d.level<100&&d.baseExp>0&&d.level%10!==0),leveling=eligible.filter(dungeonIsLeveling),pool=leveling.length?leveling:eligible;return pool.slice().sort((a,b)=>b.level-a.level||b.baseExp-a.baseExp)[0]||null}
function dungeonGainForJob(job,d){return d?Math.round(d.baseExp*armouryMultiplier(job)):0}
function renderDungeonPreview(){const box=document.getElementById('dungeon-exp-preview');if(!box)return;if(!dungeonExpCatalog.length){box.innerHTML='<span class="muted">尚無副本 EXP 資料。</span>';return}const levels=[51,61,71,81,91,99],rows=levels.map(l=>{const d=bestDungeonForLevel(l);return d?`<tr><td>Lv${l}</td><td>${lvlEsc(d.name)}</td><td>${Math.round(d.baseExp).toLocaleString('zh-TW')}</td></tr>`:''}).join('');box.innerHTML=`<table><tr><th>角色等級</th><th>自動推薦</th><th>Base EXP</th></tr>${rows}</table><div class="muted">Base EXP 先不含兵裝加成；排程時會依各職業設定自動套用。</div>`}
...
function simulateMultiJobs(inputJobs,roulettes,settings){const jobs=inputJobs.map(normalizeLevelJob).filter(j=>j.name).map(x=>({...x})),completed={},trace=[];let day=0,pointer=0,totalDungeonRuns=0;while(jobs.some(j=>!isJobDone(j))&&day<3650){const events=[],acts=schedulerDailyActivities(roulettes,day,settings.useRemainingToday);for(const a of acts){let job;if(settings.mode==='round'){const p=chooseRoundJob(jobs,pointer);job=p.job;pointer=p.pointer}else job=chooseFocusJob(jobs);if(!job)break;const before=`Lv${job.level}`,earned=addPercentBar(job,Number(a.pct)||0);events.push({kind:'daily',name:a.name,job:job.name,before,after:`Lv${job.level}`,earned});if(isJobDone(job)&&completed[job.id]==null)completed[job.id]=day+1}for(let n=0;n<(Number(settings.dungeonRuns)||0);n++){let job;if(settings.mode==='round'){const p=chooseRoundJob(jobs,pointer);job=p.job;pointer=p.pointer}else job=chooseFocusJob(jobs);if(!job)break;const d=settings.includeDungeonExp?bestDungeonForLevel(job.level):null;if(!d)break;const before=`Lv${job.level}`,earned=addAbsoluteExp(job,dungeonGainForJob(job,d));totalDungeonRuns++;events.push({kind:'dungeon',name:d.name,job:job.name,before,after:`Lv${job.level}`,earned});if(isJobDone(job)&&completed[job.id]==null)completed[job.id]=day+1}day++;if(day<=14)trace.push({day,events,jobs:jobs.map(j=>({name:j.name,level:j.level,exp:j.exp,target:j.target}))})}jobs.forEach(j=>{if(isJobDone(j)&&completed[j.id]==null)completed[j.id]=day});return{days:day,jobs,completed,trace,totalDungeonRuns}}
...
function renderMultiSchedule(){const out=document.getElementById('multi-level-result');if(!out)return;const jobs=cloneLevelJobs().filter(j=>j.name&&j.target>j.level),roulettes=getRoulettes(),settings=getLevelSchedulerSettings();if(!jobs.length){out.innerHTML='至少要有一個「目前等級低於目標」的職業。';return}const baseline=simulateMultiJobs(jobs,roulettes,settings),diffs=simulateScenarioDiffs(jobs,roulettes,settings,baseline),noDungeon=settings.dungeonRuns?simulateMultiJobs(jobs,roulettes,{...settings,dungeonRuns:0}):null;const jobRows=jobs.map(j=>{const d=baseline.completed[j.id];return`<tr><td>${lvlEsc(j.name)}</td><td>${LEVEL_ROLE_LABELS[j.role]}</td><td>Lv${j.level} → ${j.target}</td><td>${d??'—'} 天</td><td>${d?addDays(d):'—'}</td></tr>`}).join(''),diffRows=diffs.map(x=>`<tr><td>${lvlEsc(x.name)}</td><td>${x.currently?'目前有打':'目前沒打'}</td><td>${x.currently?'不打':'加入'}</td><td>${x.delta===0?'±0':x.delta>0?`+${x.delta}`:x.delta} 天</td></tr>`).join(''),dungeonLine=noDungeon?`<div class="scenario-callout">每天 ${settings.dungeonRuns} 本最高可用練等副本：總畢業約 <strong>${baseline.days} 天</strong>；若完全不刷副本約 <strong>${noDungeon.days} 天</strong>，差 ${Math.max(0,noDungeon.days-baseline.days)} 天。</div>`:'',trace=baseline.trace.slice(0,7).map(d=>`<details class="schedule-day"><summary>Day ${d.day} · ${d.events.length} 項</summary>${d.events.map(e=>`<div>${e.kind==='daily'?'🎲':'🏰'} ${lvlEsc(e.name)} → <strong>${lvlEsc(e.job)}</strong> ${e.before}→${e.after}　+${Math.round(e.earned).toLocaleString('zh-TW')} EXP</div>`).join('')||'<div class="muted">今天沒有可用項目。</div>'}</details>`).join('');out.innerHTML=`<div class="scenario-callout"><strong>全部目標：約 ${baseline.days} 天</strong>（${addDays(baseline.days)}）<br><span class="muted">模式：${settings.mode==='round'?'日隨輪替分配':'照上方順序集中練'}。日隨 % 視為平均總收益；額外刷副本使用 Wiki Base EXP，並另套兵裝加成。</span></div>${dungeonLine}<h3>各職業 ETA</h3><table><tr><th>職業</th><th>角色</th><th>目標</th><th>完成</th><th>日期</th></tr>${jobRows}</table><h3>打／不打差多少</h3><table><tr><th>項目</th><th>狀態</th><th>假設</th><th>全部目標差</th></tr>${diffRows}</table><h3>前 7 天排程預覽</h3>${trace}`;store.set('levelMultiPlan',{days:baseline.days,generatedAt:new Date().toISOString(),jobs:jobs.map(j=>({name:j.name,level:j.level,target:j.target,etaDays:baseline.completed[j.id]})),settings});renderMultiSummary()}
...
window.addEventListener('DOMContentLoaded',()=>{
  // Bind controls first. A bad saved row or a failed network request must never disable the buttons.
  ...
  safeLevelInitStep('jobs',renderLevelJobs);
  safeLevelInitStep('settings',loadSchedulerSettingsToUi);
  safeLevelInitStep('summary',renderMultiSummary);
  Promise.resolve(loadDungeonExpCatalog(false)).catch(e=>console.error('initial dungeon EXP load failed',e));
});
