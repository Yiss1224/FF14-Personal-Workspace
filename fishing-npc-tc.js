  ...
  function installRenderer(){
    if(typeof window.fgVendorLocationText==='function'){
      window.fgVendorLocationText=function(v){
        ...
        return `${zone?`<strong>${esc(zone)}</strong> · `:''}${esc(name)}${title?` &lt;${esc(title)}&gt;`:''}${v?.coords?` · X/Y ${esc(v.coords)}`:''}${area?` <span class="muted">(${esc(area)})</span>`:''}`;
      };
    }
    ...
    if(typeof window.renderVendorPlan==='function'&&typeof window.fgVendorZones==='function'){
      window.renderVendorPlan=function(){
        ...
        box.innerHTML=`<div class="vendor-plan-title">採買點合併</div>${zones.slice(0,8).map(z=>`<div class="vendor-plan-row"><strong>${esc(placeText(z.zone))}</strong><span class="badge">可處理 ${z.baits.size} 種</span>${z.npcs.map(v=>`<div>• ${esc(npcText(v))}${v.shop?` · ${esc(v.shop)}`:''}${v.coords?` · X/Y ${esc(v.coords)}`:''}${v.area?` <span class="muted">(${esc(placeText(v.area))})</span>`:''}<div class="muted">${[...new Set(v.baits)].map(esc).join('、')}</div></div>`).join('')}</div>`).join('')}`;
      };
    }
  }
