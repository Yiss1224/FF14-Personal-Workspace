...
function info(itemId,now=Date.now()){
  const f=data?.FISH?.[itemId]??data?.FISH?.[String(itemId)];if(!f)return null;
  const c=conditions(f),weatherLimited=!!(c.prev.length||c.now.length),timeUnlimited=Number(f?.startHour)===0&&Number(f?.endHour)===24,restricted=!timeUnlimited||weatherLimited,territory=territoryForFish(f),ranges=restricted?windowIntervals(f,now):[],current=ranges.find(r=>r[0]<=now&&now<r[1])||null,next=ranges.find(r=>r[0]>now)||null;
  return{itemId:Number(itemId)||0,locationId:Number(f?.location)||0,territoryId:territory,restricted,timeUnlimited,weatherLimited,startHour:Number(f?.startHour),endHour:Number(f?.endHour),current,next,waitMs:current?0:(next?next[0]-now:null),currentLeftMs:current?current[1]-now:null,nextDurationMs:next?next[1]-next[0]:null};
}
function prerequisites(itemId){const f=data?.FISH?.[itemId]??data?.FISH?.[String(itemId)];return f?predatorRows(f):[]}
function describe(itemId,now=Date.now()){const f=data?.FISH?.[itemId]??data?.FISH?.[String(itemId)],i=info(itemId,now);if(!f||!i)return null;const c=conditions(f);if(!i.restricted)return`🟢 隨時可釣 · ${etClock(now)} · 天氣無限制`;let weather='天氣無限制';if(i.weatherLimited){if(i.territoryId){const p=Math.floor((now/1000)/WEATHER_PERIOD_SEC)*WEATHER_PERIOD_SEC*1000,wid=weatherAt(i.territoryId,p+1),prev=weatherAt(i.territoryId,p-WEATHER_PERIOD_SEC*1000+1);weather=`目前 ${weatherName(wid)}`;if(c.prev.length)weather+=`（前置 ${weatherName(prev)}）`}else weather='天氣區域資料不足'}if(i.current){const total=i.current[1]-i.current[0];return`🟢 現在可釣 · ${etClock(now)} · ${weather} · 本窗 ${fmtDuration(total)} · 剩 ${fmtDuration(i.currentLeftMs)}`}if(i.next){return`⏳ ${fmtDuration(i.waitMs)}後開窗（${fmtClock(i.next[0])}）· ${weather} · 持續 ${fmtDuration(i.nextDurationMs)}`}return`⚪ 未找到未來窗口 · ${etClock(now)} · ${weather}`}
...
async function sharedDescribe(itemId,now=Date.now()){await loadData();return data?describe(itemId,now):null}
...
window.ff14FishingWindowInfo=sharedInfo;window.ff14FishingWindowDescribe=sharedDescribe;window.ff14FishingPrerequisites=sharedPrerequisites;
