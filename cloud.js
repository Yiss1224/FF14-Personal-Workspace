...
function ff14TcPlaceText(v){const s=String(v||'').trim();return ff14TcCache.places[s]||s}
function ff14TcWeatherText(v){const s=String(v||'').trim();return ff14TcCache.weather[s]||s}
function ff14TcItemText(v){const s=String(v||'').trim();return ff14TcCache.itemEnglish[s]||s}
window.ff14TcPlaceText=ff14TcPlaceText;
window.ff14TcWeatherText=ff14TcWeatherText;
window.ff14TcItemText=ff14TcItemText;
function ff14TcReplaceTextNodeWithMaps(node,maps){const raw=node.nodeValue;if(!raw||!/[A-Za-z]/.test(raw))return;let next=raw;for(const map of maps)for(const[en,tc]of Object.entries(map)){if(!en||!tc)continue;const escaped=en.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');next=next.replace(new RegExp(`(^|[^A-Za-z])(${escaped})(?=$|[^A-Za-z])`,'g'),(_,prefix)=>prefix+tc)}if(next!==raw)node.nodeValue=next}
function ff14TcReplaceTextNode(node){ff14TcReplaceTextNodeWithMaps(node,[ff14TcCache.itemEnglish,ff14TcCache.places,ff14TcCache.weather])}
function ff14TcReplaceItemTextNode(node){ff14TcReplaceTextNodeWithMaps(node,[ff14TcCache.itemEnglish])}
function ff14TcWalkWith(root,fn){const w=document.createTreeWalker(root,NodeFilter.SHOW_TEXT),nodes=[];while(w.nextNode())nodes.push(w.currentNode);nodes.forEach(fn)}
function ff14TcWalkText(root){ff14TcWalkWith(root,ff14TcReplaceTextNode)}
function ff14TcWalkItemText(root){ff14TcWalkWith(root,ff14TcReplaceItemTextNode)}
function ff14TcApplyFishRows(){const catalog=store.get('fishCatalog',[])||[],byId=new Map(catalog.map(x=>[Number(x.itemId),x]));document.querySelectorAll('#fish-catalog .fish-row').forEach(row=>{const href=row.querySelector('a[href*="/fish/"]')?.getAttribute('href')||'',m=href.match(/\/fish\/(\d+)/),id=m?Number(m[1]):0,name=row.querySelector(':scope > div:first-child > strong');if(name&&ff14TcCache.items[id]&&name.textContent!==ff14TcCache.items[id])name.textContent=ff14TcCache.items[id];const small=row.querySelector(':scope > div:first-child > small');if(small)ff14TcWalkText(small);const method=row.querySelector('.fish-method');if(method)ff14TcWalkText(method)});document.querySelectorAll('#fish-catalog details.zone > summary').forEach(s=>{const strong=s.querySelector('strong'),region=s.querySelector('span:not(.badge)');if(strong){const next=ff14TcPlaceText(strong.textContent);if(strong.textContent!==next)strong.textContent=next}if(region){const next=ff14TcPlaceText(region.textContent);if(region.textContent!==next)region.textContent=next}});document.querySelectorAll('#fish-catalog details.spot > summary').forEach(s=>{for(const n of[...s.childNodes])if(n.nodeType===Node.TEXT_NODE){const next=ff14TcPlaceText(n.nodeValue.trim())+' ';if(n.nodeValue!==next)n.nodeValue=next}})}
function ff14TcApply(){if(ff14TcApplying)return;ff14TcApplying=true;try{ff14TcLoadCache();ff14TcApplyFishRows();const bait=document.getElementById('fish-bait-list');if(bait)bait.querySelectorAll('.bait-shopping-main').forEach(ff14TcWalkItemText);const vendor=document.getElementById('fish-vendor-plan');if(vendor)vendor.querySelectorAll('.vendor-plan-row > div > .muted').forEach(ff14TcWalkItemText);const route=document.getElementById('fish-route-result');if(route)ff14TcWalkText(route)}finally{ff14TcApplying=false}}
