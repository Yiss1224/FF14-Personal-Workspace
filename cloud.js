...
// Keep the original English data as lookup keys for XIVAPI / Carbuncle / Lodinn,
// and only replace the text rendered on screen.
...
function ff14TcLoadCache(){
  try{const x=JSON.parse(localStorage.getItem(FF14_TC_CACHE_KEY)||'null');if(x&&typeof x==='object')ff14TcCache={ts:Number(x.ts)||0,items:x.items||{},places:x.places||{},weather:x.weather||{},itemEnglish:x.itemEnglish||{}}}catch{}
  return ff14TcCache
}
...
function ff14TcCsvMeta(text){
  const lines=String(text||'').replace(/^\uFEFF/,'').split(/\r?\n/),heads=ff14TcCsvLine(lines[1]||'');
  let nameIndex=heads.indexOf('Name');if(nameIndex<0)nameIndex=heads.indexOf('Singular');if(nameIndex<0)nameIndex=1;
  return{lines,nameIndex}
}
function ff14TcNamesById(text,neededIds=null){
  const {lines,nameIndex}=ff14TcCsvMeta(text),out={};
  for(let i=3;i<lines.length;i++){if(!lines[i])continue;const row=ff14TcCsvLine(lines[i]),id=Number(row[0]);if(!Number.isFinite(id))continue;if(neededIds&&!neededIds.has(id))continue;const name=String(row[nameIndex]||'').trim();if(name)out[id]=name}
  return out
}
function ff14TcPairNames(enText,tcText){
  const en=ff14TcNamesById(enText),tc=ff14TcNamesById(tcText),out={};
  for(const [id,enName] of Object.entries(en)){const tcName=tc[id];if(enName&&tcName&&enName!==tcName)out[enName]=tcName}
  return out
}
...
async function ff14TcResolveVisibleBaits(){
  if(typeof fgResolveBaitItemId!=='function'||typeof fgPathNames!=='function'||typeof fgMethodForName!=='function')return new Map();
  const names=new Set();
  for(const fish of (store.get('fishCatalog',[])||[])){
    const m=fgMethodForName(fish.name);if(!m)continue;for(const n of fgPathNames(m.bestCatchPath))if(n&&n!==fish.name)names.add(n)
  }
  const list=[...names].slice(0,180),result=new Map(),workers=6;let cursor=0;
  async function work(){while(cursor<list.length){const name=list[cursor++];try{const id=await fgResolveBaitItemId(name);if(id)result.set(name,id)}catch{}}}
  await Promise.all(Array.from({length:workers},work));return result
}
async function ff14TcRefresh(force=false){
  if(ff14TcBusy)return;ff14TcBusy=true;const button=document.getElementById('refresh-tc-terms'),status=document.getElementById('fish-catalog-status');if(button)button.disabled=true;
  try{
    ff14TcLoadCache();const fishIds=ff14TcNeededFishIds(),baitIds=await ff14TcResolveVisibleBaits(),needed=new Set([...fishIds,...baitIds.values()]);
    const missing=[...needed].some(id=>!ff14TcCache.items[id]),stale=!ff14TcCache.ts||Date.now()-ff14TcCache.ts>FF14_TC_CACHE_MS;
    if(force||missing||stale){
      if(status)status.textContent='正在建立台服繁中名詞快取…（第一次會下載台服 Item 資料）';
      const [tcItem,enPlace,tcPlace,enWeather,tcWeather]=await Promise.all([
        ff14TcFetchText(`${FF14_TC_BASE}/Item.csv`),
        ff14TcFetchText(`${FF14_EN_BASE}/PlaceName.csv`),ff14TcFetchText(`${FF14_TC_BASE}/PlaceName.csv`),
        ff14TcFetchText(`${FF14_EN_BASE}/Weather.csv`),ff14TcFetchText(`${FF14_TC_BASE}/Weather.csv`)
      ]);
      ff14TcCache.items={...ff14TcCache.items,...ff14TcNamesById(tcItem,needed)};
      ff14TcCache.places=ff14TcPairNames(enPlace,tcPlace);
      ff14TcCache.weather=ff14TcPairNames(enWeather,tcWeather);
      for(const [enName,id] of baitIds){const tc=ff14TcCache.items[id];if(tc)ff14TcCache.itemEnglish[enName]=tc}
      for(const fish of (store.get('fishCatalog',[])||[])){const tc=ff14TcCache.items[fish.itemId];if(tc)ff14TcCache.itemEnglish[fish.name]=tc}
      ff14TcCache.ts=Date.now();ff14TcSaveCache();
    }
    ff14TcApply();if(status)status.textContent=`台服繁中名詞已套用 · ${Object.keys(ff14TcCache.items).length} 個物品名稱已快取`;
  }catch(e){console.error(e);if(status)status.textContent=`台服繁中名詞讀取失敗：${e.message}`}
  finally{ff14TcBusy=false;if(button)button.disabled=false}
}
...
function ff14TcReplaceTextNode(node){
  const raw=node.nodeValue;if(!raw||!/[A-Za-z]/.test(raw))return;let next=raw;
  const maps=[ff14TcCache.itemEnglish,ff14TcCache.places,ff14TcCache.weather];
  for(const map of maps)for(const [en,tc] of Object.entries(map)){if(en&&tc&&next.includes(en))next=next.split(en).join(tc)}
  if(next!==raw)node.nodeValue=next
}
function ff14TcWalkText(root){const w=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);const nodes=[];while(w.nextNode())nodes.push(w.currentNode);nodes.forEach(ff14TcReplaceTextNode)}
function ff14TcApplyFishRows(){
