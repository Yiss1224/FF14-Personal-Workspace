...
function ff14TcApplyFishRows(){const catalog=store.get('fishCatalog',[])||[],byId=new Map(catalog.map(x=>[Number(x.itemId),x]));document.querySelectorAll('#fish-catalog .fish-row').forEach(row=>{const href=row.querySelector('a[href*="/fish/"]')?.getAttribute('href')||'',m=href.match(/\/fish\/(\d+)/),id=m?Number(m[1]):0,name=row.querySelector(':scope > div:first-child > strong');if(name&&ff14TcCache.items[id]&&name.textContent!==ff14TcCache.items[id])name.textContent=ff14TcCache.items[id];const small=row.querySelector(':scope > div:first-child > small');if(small)ff14TcWalkText(small);const method=row.querySelector('.fish-method');if(method)ff14TcWalkText(method)});document.querySelectorAll('#fish-catalog details.zone > summary').forEach(s=>{const strong=s.querySelector('strong'),region=s.querySelector('span:not(.badge)');if(strong){const next=ff14TcText(strong.textContent);if(strong.textContent!==next)strong.textContent=next}if(region){const next=ff14TcText(region.textContent);if(region.textContent!==next)region.textContent=next}});document.querySelectorAll('#fish-catalog details.spot > summary').forEach(s=>{for(const n of[...s.childNodes])if(n.nodeType===Node.TEXT_NODE){const next=ff14TcText(n.nodeValue.trim())+' ';if(n.nodeValue!==next)n.nodeValue=next}})}
